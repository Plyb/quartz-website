...
Because this is an *alignment* benchmark, not a *capabilities* benchmark, it is more important that the benchmark is run on new generations of AI.
I reimplemented MACHIAVELLI using the [Inspect](https://inspect.aisi.org.uk/) framework, reducing barriers for evaluators to use the benchmark, making it more likely that they will do so.
...
The re-implementation can be found [here](https://github.com/Plyb/inspect-machiavelli).
...
2. **Inspect is great!**
	1. It's a great example of good type hinting.
		1. Unfortunately, the original MACHIAVELLI implementation... not so much.
		   To the point that I used it as a key counterexample in my [[A Call for Better Type Hints in AI Safety Tooling|call for better type hints]]. Most of the bugs and implementation challenges I had stemmed from the fact that the original MACHIAVELLI Implementation has practically no type hints at all.
		   Now, on the one hand, do make it easier to deal with this: I can have Claude search much faster than I can to find sources of values and reason about their implicit types.
		   But on the other hand, coding agents mean you have less excuses *not* to use good type hints, as they can drastically speed up the process of writing types.
		   ...
		   That was nearly entirely written by Claude inspecting the json game data files and extracting the types.
	...
	2. It handles things like averaging over multiple runs beautifully.
		1. Although for `dict`-type scores, you do need to make sure all of the scores for all runs have the same shape.
		   MACHIAVELLI's original implementation did not guarantee this.
		   Originally, missing keys were assumed to be 0, but that broke Inspect.
